...
class Qwen3MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.similarity_method = 'cka'  # 'cka', 'cosine', 'frobenius'
        self.kernel = 'linear'  # 'linear', 'rbf', 'polynomial' (for CKA)
        self.expert_outputs = None

        # Register similarity matrix as model parameter (optional, initially None)
        self.register_buffer('similarity_matrix', None, persistent=True)

    def enable_similarity_computation(self, method='cka', kernel='linear'):
        """Enable expert similarity computation

        Args:
            method: Similarity computation method ('cka', 'cosine', 'frobenius')
            kernel: CKA kernel function type ('linear', 'rbf', 'polynomial')
        """
    ...
    def disable_similarity_computation(self):
        """Disable expert similarity computation"""
        self.compute_similarity = False
    
    def set_similarity_matrix(self, similarity_matrix):
        """Set similarity matrix"""
        if isinstance(similarity_matrix, torch.Tensor):
            self.similarity_matrix = similarity_matrix.clone().detach()
        else:
    ...
    def reset_similarity_matrix(self):
        ...
        # Force cleanup of possible cache
        if hasattr(self, 'expert_outputs'):
            self.expert_outputs = None
        # Clear CUDA cache
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # If similarity computation is needed, compute directly in forward pass
        if self.compute_similarity:
            # Collect outputs from all experts
            expert_outputs = []
            for expert_idx in range(self.num_experts):
                expert_layer = self.experts[expert_idx]
                expert_output = expert_layer(hidden_states)  # Compute for all tokens
                expert_outputs.append(expert_output)
            
            # Immediately compute similarity matrix
            device = hidden_states.device
            similarity_matrix = compute_similarity_matrix_gpu(
                expert_outputs, 
            ...
            # Move similarity matrix to CPU and save
            self.set_similarity_matrix(similarity_matrix.cpu())
            
            # Immediately clear expert outputs and similarity matrix to free GPU memory
            del expert_outputs
            del similarity_matrix
            torch.cuda.empty_cache()
            
            # Print information
            print(f"Computed similarity matrix (method: {self.similarity_method}, kernel: {self.kernel}), GPU memory cleared")
